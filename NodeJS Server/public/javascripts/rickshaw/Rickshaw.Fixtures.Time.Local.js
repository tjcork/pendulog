...
Rickshaw.Fixtures.Time.Local = function() {
	...
	this.ceil = function(time, unit) {
		...
		if (unit.name == 'day') {
			...
			var nearFuture = new Date((time + unit.seconds - 1) * milliseconds_switch);
			...
			return rounded.getTime() / milliseconds_switch;
		}

		if (unit.name == 'month') {

			date = new Date(time * milliseconds_switch);

			floor = new Date(date.getFullYear(), date.getMonth()).getTime() / milliseconds_switch;
			...
			return new Date(year, month).getTime() / milliseconds_switch;
		}
		...
		if (unit.name == 'year') {
			...
			floor = new Date(date.getUTCFullYear(), 0).getTime() / milliseconds_switch;
			if (floor == time) return time;

			year = date.getFullYear() + 1;

			return new Date(year, 0).getTime() / milliseconds_switch;
		}
		offset = new Date(time * milliseconds_switch).getTimezoneOffset() * 60;
